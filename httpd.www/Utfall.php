...
// Kontrollera om en sökfråga har skickats
    if (isset($_GET['search']) && !empty(trim($_GET['search']))) {
        ...
        $stmt = $conn->prepare("
            SELECT sh.AnswerId, sh.Name, sh.Email, sh.BestPerson, sh.SubmissionTime, sh.Gender, sh.Age, sh.Education, sh.GroupName, sh.Company, sh.InviteCode, sh.ExtroversionPercent, sh.SensingPercent, sh.ThinkingPercent, sh.JudgingPercent, sh.PersonalityType, sh.CorrectedPersonalityType, sh.ProfileCode, sh.Adjustment, sh.SubmissionTime,
            l.LanguageCode, pt1.Link AS PersonalityLink, pt2.Link AS CorrectedPersonalityLink
            FROM SurveyHeaders as sh
            LEFT JOIN Languages l ON sh.LanguageId = l.LanguageId
            LEFT JOIN PeMaTypes pt1 ON sh.PersonalityType = pt1.PeMaDescr
            LEFT JOIN PeMaTypes pt2 ON sh.CorrectedPersonalityType = pt2.PeMaDescr
            WHERE Name LIKE ? 
               OR Email LIKE ? 
               OR GroupName LIKE ? 
        ...
        <?php if ($result && $result->num_rows > 0): ?>
            <?php while ($row = $result->fetch_assoc()): ?>
                <tr>
                    ...
                    <!-- Ny kolumn med PDF-ikon -->
                    <td>
                        <?php 
                            // Samma parametrar som tidigare, men med phpWord_short.php
                            $answerId = urlencode($row['AnswerId']);
                            $personalityType = urlencode($row['PersonalityType']);
                            $name = urlencode($row['Name']);
                            $languageCode = urlencode($row['LanguageCode'] ?? 'EN'); // Standard till 'EN' om ej satt
                            $date = date('Y-m-d', strtotime($row['SubmissionTime']));
                            $date = urlencode($date);
                            $generatedShortLink = "https://proanalys.se/phpWord_short.php?ProfileCode={$personalityType}&Language={$languageCode}&Date={$date}&Name={$name}&AnswerId={$answerId}";
                        ?>
                        <a href="<?php echo htmlspecialchars($generatedShortLink); ?>" target="_blank">
                            📄
                        </a>
                    </td>                    
                    ...
                    <td>
                        <?php 
                            // Kolla om ProfileCode inte är tomt (inte NULL)
                            $addTooltip = !empty($row['ProfileCode']);
                            
                            // Bygg upp texten med asterisk om tooltip ska visas
                            $displayPersonality = htmlspecialchars($row['PersonalityType']) . ($addTooltip ? '*' : '');
                            
                            // Om tooltip ska visas, sätt title-attributet med ProfileCode
                            $tooltipAttr = $addTooltip 
                                ? ' title="' . htmlspecialchars($row['ProfileCode']) . (!empty($row['Adjustment']) ? ' ' . htmlspecialchars($row['Adjustment']) : '') . '"'
                                : '';
                    
                            // Dynamiskt generera länken
                            $personalityType = urlencode($row['PersonalityType']);
                            $name = urlencode($row['Name']);
                            $languageCode = urlencode($row['LanguageCode'] ?? 'EN'); // Standard till 'EN' om LanguageCode saknas
                            $date = date('Y-m-d', strtotime($row['SubmissionTime']));
                            $date = urlencode($date);
                            $generatedLink = "https://proanalys.se/phpWord.php?ProfileCode={$personalityType}&Language={$languageCode}&Date={$date}&Name={$name}";
                        ?>
                    
                        <?php if (!empty($row['PersonalityType'])): ?>
                            <a href="<?php echo htmlspecialchars($generatedLink); ?>" target="_blank" <?php echo $tooltipAttr; ?>>
                                <?php echo $displayPersonality; ?>
                            </a>
                        <?php else: ?>
                    ...
                    <td>
                        <?php 
                            // Kontrollera om CorrectedPersonalityType existerar
                            if (!empty($row['CorrectedPersonalityType'])):
                    
                                // Dynamiskt generera länken
                                $correctedPersonalityType = urlencode($row['CorrectedPersonalityType']);
                                $name = urlencode($row['Name']);
                                $languageCode = urlencode($row['LanguageCode'] ?? 'EN'); // Standard till 'EN' om LanguageCode saknas
                                $date = date('Y-m-d', strtotime($row['SubmissionTime']));
                                $date = urlencode($date);
                                $generatedCorrectedLink = "https://proanalys.se/phpWord.php?ProfileCode={$correctedPersonalityType}&Language={$languageCode}&Date={$date}&Name={$name}";
                        ?>
                            <a href="<?php echo htmlspecialchars($generatedCorrectedLink); ?>" target="_blank">
                                <?php echo htmlspecialchars($row['CorrectedPersonalityType']); ?>
                            </a>
                        <?php else: ?>
                            <!-- Tom kolumn om CorrectedPersonalityType är NULL -->
                        <?php endif; ?>
                    </td>
...
<style>
        .opposite-answer {
            background-color: #FFF5EB !important; /* Ljusgul bakgrund */
        }
        .opposite-answer td {
...
<div class="modal fade" id="answersModal" tabindex="-1" aria-labelledby="answersModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-xl"> <!-- Byt ut modal-lg till modal-xl -->
