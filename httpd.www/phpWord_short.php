...
// Kontrollera om $answerId inte är tomt och $name är tomt
if ($answerId !== '' && $name === '') {
    // Förbered och kör SQL-fråga
    $stmt = $conn->prepare("SELECT PersonalityType, SubmissionTime, Name FROM SurveyHeaders WHERE AnswerId = ?");
    $stmt->bind_param("s", $answerId); // Bind AnswerId som parameter
    $stmt->execute();
    $result = $stmt->get_result();

    if ($row = $result->fetch_assoc()) {
        $profileCode = $row['PersonalityType'];
...
$pdfFilePath = "onewebmedia/Pdf/Personliga/" . strtolower($profileCode) . "_sh_" . strtolower($language) . "_" . strtolower(str_replace(' ', '_', $sanitizedName)) . ".pdf";
...
$pdfLink = "https://proanalys.se/onewebmedia/Pdf/Personliga/" . strtolower($profileCode) . "_sh_" . strtolower($language) . "_" . strtolower(str_replace(' ', '_', $sanitizedName)) . ".pdf";
...
$templateFile = 'onewebmedia/WordFile/Templates/Template_short_' . $language . '.docx';
...
$docxFile = 'onewebmedia/WordFile/Personliga/' . $profileCode . '_sh_' . $language . '_' . str_replace(' ', '_', $sanitizedName) . '.docx';
...
$cloudconvertDocxUrl = "https://proanalys.se/onewebmedia/WordFile/Personliga/" . rawurlencode($profileCode . "_sh_" . $language . "_" . str_replace(' ', '_', $sanitizedName) . ".docx");
