...
// Bestäm LanguageId baserat på webbläsarens språkinställningar
$languageCode = $_POST['languageCode'] ?? 'sv'; // Standard till svenska om inget skickas
$languageId = ($languageCode === "sv") ? 1 : 2;
...
try {
    // Spara enkäthuvudet med VersionId och LanguageId
    $stmt = $conn->prepare("INSERT INTO SurveyHeaders (Name, Email, BestPerson, Gender, Age, Education, inviteCode, GroupName, Company, SubmissionTime, VersionId, LanguageId) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssisssssii", $name, $email, $best_person, $gender, $age, $education, $InviteCode, $groupName, $company, $submissionTime, $activeVersionId, $languageId);
    $stmt->execute();
