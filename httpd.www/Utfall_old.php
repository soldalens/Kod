...
// Kontrollera om en sökfråga har skickats
    if (isset($_GET['search']) && !empty(trim($_GET['search']))) {
        ...
        $stmt = $conn->prepare("
            SELECT sh.AnswerId, sh.Name, sh.Email, sh.BestPerson, sh.SubmissionTime, sh.Gender, sh.Age, sh.Education, sh.GroupName, sh.Company, sh.InviteCode, sh.ExtroversionPercent, sh.SensingPercent, sh.ThinkingPercent, sh.JudgingPercent, sh.PersonalityType, sh.CorrectedPersonalityType, sh.ProfileCode, sh.Adjustment,
            pt1.Link AS PersonalityLink, pt2.Link AS CorrectedPersonalityLink
        ...
        $result = $stmt->get_result(); // Hämta resultaten
            ...
            $result = $conn->query("SELECT sh.AnswerId, sh.Name, sh.Email, sh.BestPerson, sh.SubmissionTime, sh.Gender, sh.Age, sh.Education, sh.GroupName, sh.Company, sh.InviteCode, sh.ExtroversionPercent, sh.SensingPercent, sh.ThinkingPercent, sh.JudgingPercent, sh.PersonalityType, sh.CorrectedPersonalityType, sh.ProfileCode, sh.Adjustment,
            pt1.Link AS PersonalityLink, pt2.Link AS CorrectedPersonalityLink
            FROM SurveyHeaders as sh
            LEFT JOIN PeMaTypes pt1 ON sh.PersonalityType = pt1.PeMaDescr
            LEFT JOIN PeMaTypes pt2 ON sh.CorrectedPersonalityType = pt2.PeMaDescr
        ...
        <?php if ($result && $result->num_rows > 0): ?>
            <?php while ($row = $result->fetch_assoc()): ?>
                <tr>
                        ...
                        <?php 
                            ...
                            // Om tooltip ska visas, sätt title-attributet med ProfileCode
                                $tooltipAttr = $addTooltip 
                                    ...
                                    : '';
                        ?>
                        <?php if (!empty($row['PersonalityLink'])): ?>
                            <a href="<?php echo htmlspecialchars($row['PersonalityLink']); ?>" target="_blank" <?php echo $tooltipAttr; ?>>
                    ...
                    <td>
                        <?php if (!empty($row['CorrectedPersonalityType'])): ?>
                            <?php if (!empty($row['CorrectedPersonalityLink'])): ?>
                                <a href="<?php echo htmlspecialchars($row['CorrectedPersonalityLink']); ?>" target="_blank">
