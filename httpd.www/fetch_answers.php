...
// Hämta `PersonalityType`, `Name` och `LanguageId` från SurveyHeaders
$surveyHeaderStmt = $conn->prepare("SELECT PersonalityType, Name, LanguageId FROM SurveyHeaders WHERE AnswerId = ?");
...
$languageId = $surveyHeaderRow['LanguageId'] ?? 1; // Standard till 1 om ej satt
...
$questionTextColumn = ($languageId == 1) ? "q.QuestionText" : "q.QuestionTextENG";
$answerTextColumn   = ($languageId == 1) ? "ao.AnswerText"   : "ao.AnswerTextENG";
$otherAnswerTextColumn = ($languageId == 1) ? "ao2.AnswerText" : "ao2.AnswerTextENG";

// SQL-fråga för att hämta frågesvar med språkberoende kolumner
$sql = "
    SELECT 
        q.OrderNumber, 
        $questionTextColumn AS QuestionText,
        $answerTextColumn AS SelectedAnswer, 
        r.ResponseDescr, 
        GROUP_CONCAT(DISTINCT $otherAnswerTextColumn SEPARATOR ', ') AS OtherAnswers,
    ...
    WHERE h.AnswerId = ?
    GROUP BY q.OrderNumber, $questionTextColumn, $answerTextColumn, r.ResponseDescr
